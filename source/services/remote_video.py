...
def materialize_remote_video(url: str, work_dir: Path) -> Path:
    ...
    AquaMetric V12.2 treats remote players such as YouTube as reference media only:
    embed + exact timestamp/bookmark. Pixel analysis, OCR, screenshots and downloadable
    clips require a video file explicitly uploaded by the user. Keeping this guard in
    the legacy helper prevents any historical route from reintroducing extraction.
    """
    del work_dir
