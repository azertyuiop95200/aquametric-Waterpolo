...
def run_complete_analysis(db, match, upload_dir: Path, evidence_dir: Path, *, include_audio: bool = True):
    """Run dense Vision/OCR only on a video file explicitly uploaded to AquaMetric.

    Third-party players such as YouTube remain reference-only: AquaMetric embeds the
    source and can keep timestamp/bookmark evidence, but it never downloads/copies
    those pixels on the server. This avoids bot/cookie extraction failures and keeps
    third-party evidence provenance explicit.
    """
