...
def player_biography_context(db, profile, scout_rows=None):
    ...
    career = list(curated.get("career", [])); honours = list(curated.get("honours", [])); highlights = list(curated.get("highlights", []))
    former_clubs = [x for x in career if x["kind"] == "former_club"]
    gaps = []
    if not former_clubs:
        gaps.append({"key": "profile.gap.former_clubs", "text": "Former water-polo clubs are not yet verified from a reliable source."})
    if not honours:
