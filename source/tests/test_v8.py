...
def test_transfer_watch_seeds_current_sources_and_signals():
    ...
    with SessionLocal() as db: assert db.scalar(select(SourceWatch).where(SourceWatch.platform=='facebook')) is not None; assert db.scalar(select(TransferSignal).where(TransferSignal.player_name=='Kamilla Farago',TransferSignal.signal_type=='confirmed')) is not None
