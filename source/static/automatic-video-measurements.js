...
function vision(v){
 if(!v)return `<div class="aqm-panel"><h3>Mesures automatiques vidéo</h3><div class="aqm-note warn"><b>Aucun scan Vision disponible</b><div>Sur une vidéo locale, l’analyse complète doit d’abord produire le scan visuel/OCR.</div></div></div>`;
 return `<div class="aqm-panel" id="aqm-auto-video"><h3>Mesures automatiques réellement extraites de la vidéo</h3><p>Ces valeurs viennent du fichier vidéo et du moteur Vision, indépendamment du tagging manuel des actions.</p><div class="aqm-kpis">${kpi('Durée analysée',`${v.duration_minutes} min`,`${v.duration_seconds} s`)}${kpi('Cadence vidéo',`${v.fps} fps`,`${v.width}×${v.height}`)}${kpi('Images analysées',v.sample_count,`intervalle ≈ ${v.sample_interval_seconds} s`)}${kpi('Type vidéo',v.video_type,`confiance ${v.confidence}`)}${kpi('Activité estimée',`${v.active_minutes_estimate} min`,`${v.active_windows_count} fenêtres actives`)}${kpi('Motion moyenne',v.avg_motion_score,'score visuel relatif')}${kpi('Présence bassin',v.avg_pool_ratio,'ratio visuel moyen')}${kpi('Coupes scène',v.scene_cut_rate,'taux relatif')}${kpi('Moments intéressants',v.interesting_moments_count,'candidats visuels')}${kpi('Zones scoreboard',v.scoreboard_roi_candidates,'candidats ROI')}</div></div>`;
}
function autonomy(a){
